	...
	switch($action)
		{
			...
			case "check":
				...
				if (empty($add))
				{
					$query = $db->prepare('SELECT ami_from, ami_date, membre_pseudo FROM forum_ami LEFT JOIN forum_membres ON membre_id = ami_from WHERE ami_to = :id AND ami_confirm = :conf ORDER BY ami_date DESC');
					$query->bindValue(':id',$id,PDO::PARAM_INT);
					$query->bindValue(':conf','0',PDO::PARAM_STR);
					$query->execute();
					...
					{
					echo '<tr><td><a href="./voirprofil.php?m='.$data['ami_from'].'&amp;action=consulter">'.stripslashes(htmlspecialchars($data['membre_pseudo'])).'</a><td>'.date('d/m/Y',$data['ami_date']).'</td>
						<td><a href="./amis.php?action=check&amp;add=ok&amp;m='.$data['ami_from'].'">Accepter</a>-<a href="./amis.php?action=delete&amp;m='.$data['ami_from'].'">Refuser</a>
						</td></tr>';
					}
					$query->CloseCursor();
				}
				...
				if (!isset($_GET['ok']))
				{
					...
					<a href="./amis.php?action=delete&amp;ok=ok&amp;m='.$membre.'">oui</a> - <ahref="./amis.php">non</a></p>';
				}
				else
				{
					...
					$query->execute();
					$query->closeCursor();
					$query = $db->prepare('DELETE FROM forum_amis WHERE ami_to =
					:membre AND ami_from = :id');
					$query->bindValue(':membre',$membre,PDO::PARAM_INT);
					$query->bindValue(':id',$id,PDO::PARAM_INT);
					$query->execute();
					$query->closeCursor();
					echo '<p>Membre correctement supprimé  <br />
					Cliquez <a href="./amis.php">ici</a> pour retourner à la liste des amis</p>';
				}
				break;
				default:
					$query = $db->prepare('SELECT (ami_from + ami_to - :id) AS ami_id, ami_date, membr FROM forum_amis LEFT JOIN forum_membres ON membre_id = (ami_from + ami_to - :id) LEFT JOIN forum_whosonline ON online_id = membre_id WHERE (ami_from = :id OR ami_to = :id) AND ami_confirm = :conf ORDER BY membre_pseudo');
					$query->bindValue(':id',$id,PDO::PARAM_INT);
					$query->bindValue(':conf','1',PDO::PARAM_STR);
					$query->execute();
					echo '<table align="center"><tr>
					...
					while ($data = $query->fetch())
					{
						echo '<tr><td><a href="./voirprofil.php?m='.$data['ami_id'].'&amp;action=consulter">'.stripslashes(htmlspecialchars($data['membre_pseudo'])).'</a></td><td>'.date('d/m/Y',$data['ami_date']).'</td>
						<td><a href="./messagesprives.php?action=repondre&amp;dest='.$data['ami_id'].'">Envoyez<a href="./amis.php?action=delete&m='.$data['ami_id'].'">Supprimer</a></td>';
		...
		<li><a href="./amis.php?action=check">Voir les demandes d\'ajout ('.$demande_ami.')</a>';
					break;
		}?>