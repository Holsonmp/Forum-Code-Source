...
<div class="container">
<?php
	echo '<p><i>Vous êtes ici</i> : <a href="./index.php">Accueil </a>--><a href="./forum.php">forum</a> --><a href="./voirforum.php?f='.$forum.'">'.stripslashes(htmlspecialchars($data['forum_name'])).'</a>';
		//Nombre de pages
...
<?php
		...
		{
			echo 'Vous ne pouvez pas faire cette action <img src="./images/lock.gif"/>, Inscrivez-vous ou essayez de vous connecter';
		}
		$query->CloseCursor();
?>

<?php
			$query=$db->prepare('SELECT forum_topic.topic_id, topic_titre,
			...
			FROM forum_topic
			LEFT JOIN forum_membres Mb ON Mb.membre_id =
			forum_topic.topic_createur
			LEFT JOIN forum_post ON forum_topic.topic_last_post =
			forum_post.post_id
			LEFT JOIN forum_membres Ma ON Ma.membre_id =
...
?>
				<table class="table table-bordered table-striped table-condensed">
					<tr class="success">
						...
						<th class="table-header"><strong>Titre</strong></th>
			...
			while ($data=$query->fetch())
				{
					...
					echo'
						<tr class="success">
							<td><tr><td><img src="./images/'.$ico_mess.'"></td></td>
							<td class="header"><strong>Annonce : </strong><strong><a href="./voirtopic.php?t='.$data['topic_id'].'" title="Topic commencé à '.date('H\hi \l\e d M,y',$data['topic_time']).'">'.stripslashes(htmlspecialchars($data['topic_titre'])).'</a></strong></td>
...
?>
						<table class="table table-bordered table-striped table-condensed">
							<tr>
								<th><img src="./images/message.png" alt="Message" /></th>
								<th class="header"><strong>Titre</strong></th>
								<th class="nombremessages"><strong>Réponses</strong></th>
								<th class="nombrevu"><strong>Vus</strong></th>
								<th class="auteur"><strong>Auteur</strong></th>
								<th class="derniermessage"><strong>Dernier message </strong></th>
...
<?php
						//On lance la boucle
								while ($data = $query->fetch())
									{	
										echo'
										<tr class="danger"><td><img src="./images/message.png" alt="Message" /></td>
										...
										<td class="nombremessages">'.$data['topic_post'].'</td>
										<td class="nombrevu">'.$data['topic_vu'].'</td><td><a href="./voirprofil.php?m='.$data['topic_createur'].'&amp;action=consulter">'.stripslashes(htmlspecialchars($data['membre_pseudo_createur'])).'</a></td>';
										//Selection dernier message
										$nombreDeMessagesParPage = 15;
										$nbr_post = $data['topic_post'] +1;
										$page = ceil($nbr_post / $nombreDeMessagesParPage);
										echo '
											<td class="warning">Par <a href="./voirprofil.php?m='.$data['post_createur'].'&amp;action=consulter">'.stripslashes(htmlspecialchars($data['membre_pseudo_last_posteur'])).'</a><br/>
...
<?php
					...
					{
						echo'<p>Ce forum ne contient aucun sujet actuellement <img src="./images/smileys/lol.gif"></p>';
					}
					$query->CloseCursor();
?>
</div>
