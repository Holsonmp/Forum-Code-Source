...
echo'<p><i>Vous êtes ici</i> : <a href="./index.php">Accueil</a> --> <a href="./admin.php">Administration du forum</a>';
...
switch($cat) //1er switch
	{
		case "config":
			...
			{
				echo '<p><label for='.$data['config_nom'].'>'.$config_name[$data['config_nom']].'
				</label> :
	...
	switch($action) //2eme switch
		...
		//Edition d'un forum
			...
			if(!isset($_GET['e']))
			{
				...
				<a href="./admin.php?cat=forum&action=edit&amp;e=editf">Editer un forum</a><br />
				<a href="./admin.php?cat=forum&action=edit&amp;e=editc">Editer une catégorie</a><br />
				<a href="./admin.php?cat=forum&action=edit&amp;e=ordref">Changer l ordre des forums</a><br />
				<a href="./admin.php?cat=forum&action=edit&amp;e=ordrec">Changer l ordre des catégories</a>
				<br /></p>';
			}
			elseif($_GET['e'] == "editf")
			{
				...
				else
				{
					...
					<label>Nom du forum : </label><input type="text" id="nom" name="nom" value="'.$data1['forum_name'].'" />
					<br />
					<label>Description :</label><textarea cols=40 rows=4 name="desc" id="desc">'.$data1['forum_desc'].'</textarea><br /><br />';
					$query->CloseCursor();
			...
			elseif($_GET['e'] == "editc")
			{
				...
				else
				{
					...
					<br /><br />
					<input type="hidden" name="cat" value="'.$_POST['cat'].'" /><input type="submit" value="Envoyer" /></p></form>';
					$query->CloseCursor();
				}
			}
		...
		<br />
		<a href="./admin.php?cat=forum&amp;action=edit">Modifier un
		forum</a>
		<br />
...
<?php
			...
			case "droits":
				...
				if(!isset($_POST['membre']))
				{
					...
					<p><label for="membre">Inscrivez le pseudo : </label><input type="text" id="membre" name="membre"><input type="submit" value="Chercher"></p></form>';
				}
				else
				{
					$pseudo_d = $_POST['membre'];
					...
					if ($data = $query->fetch())
					{
						...
						echo'</select><input type="hidden" value="'.stripslashes($pseudo_d).'"name="pseudo">
						<input type="submit" value="Envoyer"></form>';
						$query->CloseCursor();
					}
					else echo' <p>Erreur : Ce membre n existe pas, <br />
			...
			case "ban":
				...
				$query = $db->query('SELECT membre_id, membre_pseudo FROM forum_membres WHERE membre_rang = 0');//Bien sur, on ne lance la suite que s'il y a des membres bannis !
				if ($query->rowCount() > 0)
				{
					while($data = $query->fetch())
					{
						echo'<br /><label><a href="./voirprofil.php?action=consulter&amp;m='.$data['membre_id'].'">'.stripslashes(htmlspecialchars($data['membre_pseudo'])).'</a></label>
						<input type="checkbox" name="'.$data['membre_id'].'" />Débannir<br />';
					}
			...
			echo'<p>Salut, alors tu veux faire quoi ?<br />
	...
	<a href="./admin.php?cat=membres">Administration des membres</a><br
	/></p>';
	break;
	}
?>