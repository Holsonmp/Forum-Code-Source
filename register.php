...
{
echo '<div class="container"><h1>Inscription 1/2</h1>';
?>
	<form method="post" class="form col-md-5" action="register.php" enctype="multipart/form-data">
		<fieldset>
			<legend>Identifiants</legend>
				<label for="pseudo">* Pseudo :</label> <input name="pseudo" type="text" id="pseudo" requierd /> (le pseudo doit contenir entre 3 et 15 caractères)<br />
				<label for="password">* Mot de Passe :</label><input type="password" name="password" id="password" requierd /><br />
				<label for="confirm">* Confirmer le mot de passe :</label><input type="password" name="confirm" id="confirm" requierd />
		</fieldset>
		<fieldset>
			<legend>Contacts</legend>
				<label for="email">* Votre adresse Mail :</label><input type="text" name="email" id="email" requierd /><br />
		...
		<label for="avatar">Choisissez votre avatar :</label><input type="file" name="avatar" id="avatar" class="6u 12u(narrower)" />(Taille max : 10Ko<br />
...
<?php
			...
			//On récupère les variables
					...
					if ($pass != $confirm || empty($confirm) || empty($pass))
						...
						if (!preg_match("#^[a-zA-Z0-9._-]+@[a-zA-Z0-9._-]{2,}\.[az]{2,4}$#", $msn) && !empty($msn))
						{
			...
			?>
				...
				if ($i==0)
					{
						...
						//La ligne suivante sera commentée plus bas
							...
							$query=$db->prepare('INSERT INTO forum_membres (membre_pseudo, membre_mdp, membre_email, membre_msn, membre_siteweb, membre_avatar, membre_signature, membre_localisation, membre_inscrit, membre_derniere_visite) VALUES (:pseudo, :pass, :email, :msn, :website, :nomavatar, :signature, :localisation, :temps, :temps)');
							$query->bindValue(':pseudo', $pseudo, PDO::PARAM_STR);
				...
				{
					echo'<h1 class="form-control warring">Inscription interrompue</h1>';
					echo'<p>Une ou plusieurs erreurs se sont produites pendant votre incription</p>';
					echo'<p>'.$i.' erreur(s)</p>';
